...
public record CreateScenarioRequest(
	...
	public CreateScenarioUseCaseDto toUseCaseDto() {
		return new CreateScenarioUseCaseDto(
			...
			conversationStates != null
				...
				.map(s -> new CreateScenarioUseCaseDto.ConversationStateDto(s.stateOrder(), s.stateName()))
				.toList()
				: null,
			conversationSlots != null
				? conversationSlots.stream()
				.map(s -> new CreateScenarioUseCaseDto.ConversationSlotDto(s.slotKey(), s.allowedValues()))
				.toList()
				: null
		);
	}
