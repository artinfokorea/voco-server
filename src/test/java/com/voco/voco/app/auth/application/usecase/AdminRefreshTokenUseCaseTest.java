...
class AdminRefreshTokenUseCaseTest {
	...
	class Execute {
		...
		void adminRefresh_Success() {
			...
			willDoNothing().given(adminChecker).validateAdmin(MEMBER_ID);
			...
			// when
			TokenInfo result = adminRefreshTokenUseCase.execute(dto);
		...
		void adminRefresh_MemberIdMismatch_ThrowsException() {
			// given
			RefreshTokenUseCaseDto dto = new RefreshTokenUseCaseDto(OLD_ACCESS_TOKEN, OLD_REFRESH_TOKEN);
			...
			// when & then
			assertThatThrownBy(() -> adminRefreshTokenUseCase.execute(dto))
			...
			then(adminChecker).shouldHaveNoInteractions();
			then(tokenQueryRepository).shouldHaveNoInteractions();
		}

		@Test
		@DisplayName("관리자가 아니면 예외가 발생한다")
		void adminRefresh_NotAdmin_ThrowsException() {
			// given
			RefreshTokenUseCaseDto dto = new RefreshTokenUseCaseDto(OLD_ACCESS_TOKEN, OLD_REFRESH_TOKEN);

			given(jwtAdaptor.extractMemberIdIgnoreExpiration(dto.accessToken())).willReturn(MEMBER_ID);
			given(jwtAdaptor.extractMemberId(dto.refreshToken())).willReturn(MEMBER_ID);
			willThrow(new CoreException(ApiErrorType.ADMIN_ONLY)).given(adminChecker).validateAdmin(MEMBER_ID);
			...
			assertThatThrownBy(() -> adminRefreshTokenUseCase.execute(dto))
				...
				.satisfies(exception -> {
					CoreException coreException = (CoreException)exception;
					assertThat(coreException.getErrorType()).isEqualTo(ApiErrorType.ADMIN_ONLY);
				});
		...
		void adminRefresh_TokenNotFound_ThrowsException() {
			// given
			RefreshTokenUseCaseDto dto = new RefreshTokenUseCaseDto(OLD_ACCESS_TOKEN, OLD_REFRESH_TOKEN);
		...
		void adminRefresh_RefreshTokenMismatch_ThrowsException() {
			// given
			RefreshTokenUseCaseDto dto = new RefreshTokenUseCaseDto(OLD_ACCESS_TOKEN, OLD_REFRESH_TOKEN);
			TokenEntity token = mock(TokenEntity.class);
		...
		void adminRefresh_RefreshTokenExpired_ThrowsException() {
			// given
			RefreshTokenUseCaseDto dto = new RefreshTokenUseCaseDto(OLD_ACCESS_TOKEN, OLD_REFRESH_TOKEN);
			TokenEntity token = mock(TokenEntity.class);
