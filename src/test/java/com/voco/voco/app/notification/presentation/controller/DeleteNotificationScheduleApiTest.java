...
@Transactional
@DisplayName("DELETE /api/v1/notification-schedules/{id}")
class DeleteNotificationScheduleApiTest {
	...
	@Test
	@DisplayName("유효한 요청으로 알림 스케줄 삭제에 성공한다")
	void deleteNotificationSchedule_Success() throws Exception {
		// given
		Long scheduleId = createSchedule(DayOfWeek.MONDAY, LocalTime.of(9, 0));

		// when
		ResultActions result = mockMvc.perform(delete(NOTIFICATION_SCHEDULE_URL + "/" + scheduleId)
			.header("Authorization", "Bearer " + accessToken));
	...
	void deleteNotificationSchedule_SoftDelete_NotFound() throws Exception {
		// given
		Long scheduleId = createSchedule(DayOfWeek.MONDAY, LocalTime.of(9, 0));
	...
	@Test
	@DisplayName("존재하지 않는 스케줄 삭제 시 실패한다")
	void deleteNotificationSchedule_NotFound_Fail() throws Exception {
		// given
		Long nonExistentId = 99999L;

		// when
		ResultActions result = mockMvc.perform(delete(NOTIFICATION_SCHEDULE_URL + "/" + nonExistentId)
			.header("Authorization", "Bearer " + accessToken));
	...
	@Test
	@DisplayName("다른 사용자의 스케줄 삭제 시 실패한다")
	void deleteNotificationSchedule_OtherUserSchedule_Fail() throws Exception {
		// given
		Long scheduleId = createSchedule(DayOfWeek.MONDAY, LocalTime.of(9, 0));
	...
	@Test
	@DisplayName("토큰이 없으면 알림 스케줄 삭제에 실패한다")
	void deleteNotificationSchedule_NoToken_Fail() throws Exception {
		// given
		Long scheduleId = createSchedule(DayOfWeek.MONDAY, LocalTime.of(9, 0));
